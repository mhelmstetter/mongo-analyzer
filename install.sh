...
# Mongo Analyzer Installation Script for Mac/Linux
# This script installs mongo-analyzer to ~/.local/bin and sets up necessary configurations
...
print_color $BLUE "┌─────────────────────────────────────────────┐"
print_color $BLUE "│          Mongo Analyzer Installer           │"
print_color $BLUE "│    MongoDB Statistics & Index Analysis      │"
print_color $BLUE "└─────────────────────────────────────────────┘"
echo
...
if ! command -v java &> /dev/null; then
    ...
    print_color $YELLOW "Please install Java 11 or later and try again."
...
if [[ $JAVA_VERSION -lt 11 ]]; then
    print_color $RED "Error: Java 11 or later is required. Found version: $JAVA_VERSION"
    exit 1
fi
...
if [ -f "$INSTALL_DIR/$SCRIPT_NAME" ]; then
    ...
    print_color $BLUE "Installing Mongo Analyzer..."
fi
...
# Mongo Analyzer wrapper script
# This script was generated by install.sh in $SCRIPT_DIR
JAR_FILE="$JAR_ABS_PATH"

if [[ ! -f "\$JAR_FILE" ]]; then
    echo "Error: Mongo Analyzer JAR file not found at \$JAR_FILE"
    echo "The JAR may have been moved or deleted."
    echo "Please run 'mvn clean package' in $SCRIPT_DIR or run install.sh again."
    exit 1
fi
...
print_color $GREEN "✓ Mongo Analyzer installed successfully!"
...
if [[ ":$PATH:" == *":$INSTALL_DIR:"* ]]; then
    ...
    if command -v mongo-analyzer &> /dev/null; then
        print_color $GREEN "✓ mongo-analyzer command is available"
        echo
    fi
else
...
print_color $BLUE "  mongo-analyzer --help                      # Show usage and options"
print_color $BLUE "  mongo-analyzer                             # Analyze all databases"
print_color $BLUE "  mongo-analyzer -d mydb                     # Analyze specific database"
print_color $BLUE "  mongo-analyzer -o html                     # Generate HTML report"
echo
print_color $BLUE "Examples:"
print_color $BLUE "  # Analyze local MongoDB with table output"
print_color $BLUE "  mongo-analyzer"
echo
print_color $BLUE "  # Analyze Atlas cluster with HTML report"
print_color $BLUE "  mongo-analyzer -c \"mongodb+srv://user:pass@cluster.mongodb.net/\" -o html"
echo
print_color $BLUE "  # Analyze specific database with JSON output"
print_color $BLUE "  mongo-analyzer -d production -o json"
echo
print_color $BLUE "  # Analyze with collection filtering"
print_color $BLUE "  mongo-analyzer -d mydb --include-collections \"users,orders\""
echo
print_color $GREEN "Mongo Analyzer is ready to use!"
